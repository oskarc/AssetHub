...
public sealed class WebhookRepository(AssetHubDbContext db) : IWebhookRepository
{
    public async Task<List<Webhook>> ListAllAsync(CancellationToken ct = default)
        => await db.Webhooks.AsNoTracking().OrderByDescending(w => w.CreatedAt).ToListAsync(ct);
